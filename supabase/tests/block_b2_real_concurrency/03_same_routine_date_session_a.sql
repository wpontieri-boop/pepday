-- Sessão A é obrigatoriamente a vencedora e mantém o lock por 20 s.
begin;
...
insert into public.audit_logs(user_id,action,operation_id,created_at) values (
  ...
  case when current_setting('pepday.b2.outcome')='PASS' then 'b2c_s3_a_acquired_pass' else 'b2c_s3_a_acquired_fail' end,
  'a1630000-0000-4000-8000-000000000001',current_setting('pepday.b2.acquired_at')::timestamptz);
select pg_sleep(case when current_setting('pepday.b2.outcome')='PASS' then 20 else 0 end);
insert into public.audit_logs(user_id,action,operation_id,created_at) values (
  'f2600000-0000-4000-8000-000000000001',
  case when current_setting('pepday.b2.outcome')='PASS' then 'b2c_s3_a_releasing_pass' else 'b2c_s3_a_releasing_fail' end,
  'a1630000-0000-4000-8000-000000000002',clock_timestamp());
commit;
