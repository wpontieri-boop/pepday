-- Sessão B: a RPC precisa aguardar o FOR UPDATE de A por pelo menos 8 s.
begin;
...
do $$ begin
  perform public.register_application(
    'b2620000-0000-4000-8000-000000000001',auth.uid(),
    'd2620000-0000-4000-8000-000000000001','c2620000-0000-4000-8000-000000000001',
    'e2620000-0000-4000-8000-000000000002',current_date,null);
...
insert into public.audit_logs(user_id,action,operation_id,created_at) values
  ('f2600000-0000-4000-8000-000000000001','b2c_s2_b_started',
    'b1620000-0000-4000-8000-000000000001',current_setting('pepday.b2.started_at')::timestamptz),
  ('f2600000-0000-4000-8000-000000000001',
    case when current_setting('pepday.b2.outcome')='PASS'
      ...
      then 'b2c_s2_b_finished_pass' else 'b2c_s2_b_finished_fail' end,
    'b1620000-0000-4000-8000-000000000002',current_setting('pepday.b2.finished_at')::timestamptz);
commit;
