...
begin
  if exists (
    ...
    raise exception 'ABORT: um UUID reservado do smoke B2.1 já existe';
  end if;
end $$;
...
select public.complete_onboarding('B2 real trial','BR','America/Sao_Paulo',true,
...
select public.complete_onboarding('B2 real pro','BR','America/Sao_Paulo',true,
  'b2-real-smoke','b2-real-smoke',false);
select set_config('request.jwt.claim.sub','f2400000-0000-4000-8000-000000000004',true);
select public.complete_onboarding('B2 real other','BR','America/Sao_Paulo',true,
  'b2-real-smoke','b2-real-smoke',false);
reset role;
...
select set_config('pepday.b2_real_trial_application',id::text,true)
...
select 'PASS — B2.1 real smoke; a próxima instrução é ROLLBACK' as resultado;
