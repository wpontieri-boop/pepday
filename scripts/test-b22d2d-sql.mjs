// Executa B2.2-D2-D em PostgreSQL efemero; nunca conecta ao Supabase.
...
try {
  ...
  for (const file of [
    'supabase/tests/block_b22d2d_soft_delete_routines.sql',
    'supabase/tests/block_b22d2c_update_routines.sql',
    'supabase/tests/block_b22d2b_create_routines.sql',
  ]) {
    currentFile = file;
    await db.exec(await read(file));
  }

  // Fixtures minimas exigidas pelas regressoes anteriores no schema final.
  currentFile = 'fixtures de regressao D2-A/D1';
  await db.exec(`insert into auth.users(id,email) values
      ('d2a00000-0000-4000-8000-000000000001','d2a-after-d2d@example.invalid'),
      ('d1f00000-0000-4000-8000-000000000001','d1-after-d2d@example.invalid');
    insert into public.vials(id,user_id,name,initial_mg,remaining_mg,water_ml,prepared_on) values
      ('d2a00000-0000-4000-8000-000000000002','d2a00000-0000-4000-8000-000000000001',
        'D2-A apos D2-D',10,10,2,date '2026-09-25'),
      ('d1f00000-0000-4000-8000-000000000002','d1f00000-0000-4000-8000-000000000001',
        'D1 apos D2-D',10,10,2,date '2026-09-25');
    ...
    values('d2a00000-0000-4000-8000-000000000003','d2a00000-0000-4000-8000-000000000001',
      'd2a00000-0000-4000-8000-000000000002','Rotina D2-A apos D2-D',1,'mg',100,
  ...
  console.log('PASS: B2.2-D2-D e regressoes D2-C, D2-B, D2-A, D1, B2.1, B1 e importacao legada.');
} catch (error) {
  console.error(`${currentFile}: ${error.message}`);
  if (error.detail) console.error(error.detail);
  if (error.position) console.error(`posicao SQL: ${error.position}`);
  process.exitCode = 1;
} finally {
  await db.close();
}
